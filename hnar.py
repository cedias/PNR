...
def train(epoch,net,optimizer,dataset,criterion,cuda,optimize=False,msg="test"):
    ...
    with tqdm(total=len(dataset),desc=msg,ncols=70) as pbar:
        for iteration, (batch_t,r_t,u_t,i_t,sent_order,ui_indexs,ls,lr,review) in enumerate(dataset):
            ...
            sent ,pred_r ,_,_,_ = net(data[0],data[2],data[3],data[4],data[5],ls,lr)

            ok,per,_ = accuracy(sent,data[1])
            ok_all[0] += ok.data[0]
            mseloss_r = F.mse_loss(pred_r,data[1].float(),size_average=False)
            mean_mse[1] += mseloss_r.data[0]

            pred_loss_s =  criterion(sent, data[1])
            
            loss = 1*torch.mean(mseloss_r) + torch.mean(pred_loss_s) #+  0.01 * F.relu(reg-1.5) 
...
def load(args):
    ...
    _,per = data_tl.get_stats("rating",trainit,True)

    w = [per[x] for x in sorted(list(per.keys()))]

    if args.load:
        net = HNAR(ntoken=len(state["word_dic"]),nusers=state["users.weight"].size(0), nitems=state["items.weight"].size(0),emb_size=state["embed.weight"].size(1),hid_size=state["sent.rnn.weight_hh_l0"].size(1),num_class=state["lin_out.weight"].size(0))
        del state["word_dic"]
        net.load_state_dict(state)

    else:
        if args.emb:
            net = HNAR(ntoken=len(wdict),nusers=len(user_mapping), nitems=len(item_mapping),emb_size=len(tensor[1]),hid_size=args.hid_size,num_class=num_classes)
            net.set_emb_tensor(torch.FloatTensor(tensor))
        else:
            net = HNAR(ntoken=len(wdict),nusers=len(user_mapping), nitems=len(item_mapping), emb_size=args.emb_size,hid_size=args.hid_size, num_class=num_classes)

    if args.prebuild:
        data_tl = FMTL(list(x for x  in tqdm(data_tl,desc="prebuilding")),data_tl.rows)

    return data_tl,(trainit,valit,testit), net, wdict, w


def main(args):

    print(32*"-"+"\nHierarchical Neural Attention for Recommendation :\n" + 32*"-")
    data_tl, (train_set, val_set, test_set), net, wdict, w = load(args)
    ...
    dataloader_valid = DataLoader(data_tl.indexed_iter(val_set), batch_size=64, shuffle=False,  num_workers=3, collate_fn=tuple_batch, pin_memory=True)
    dataloader_test = DataLoader(data_tl.indexed_iter(test_set), batch_size=64, shuffle=False, num_workers=3, collate_fn=tuple_batch, pin_memory=True)

    criterion = torch.nn.CrossEntropyLoss(weight=torch.Tensor(w).cuda(),size_average=False,reduce=False)      
