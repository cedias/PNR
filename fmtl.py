...
class FMTL():
    ...
    def __init__(self, tuplelist,rows=None):
        ...
        self.unknown = {}
        self.rows = rows
        self.prehook,self.posthook = lambda x:x,lambda x:x
    ...
    def __getitem__(self,index):
        ...
        Maps field[x]:
            ...
            (see self._rec_apply)
        """
        t = self.prehook(list(self.tuplelist[index]))

        if len(self.mappings) == 0:
            return tuple(self.posthook(t))
        else:
    ...
    def get_stats(self, field, key_iter=None, verbose=False):
        ...
        """
        field = self._f2i(field)
        d =  dict(Counter(tqdm(self.field_gen(field,key_iter=key_iter),desc="computing stats",total=len(self))))
